        ...
        <>
            {isDesktop
                ? filtered_items.map(({ as, href, icon, label }) => {
                    const is_active = window.location.pathname === href || (href === '/' && (window.location.pathname === '/' || window.location.pathname === ''));
                    return (
                        <MenuItem
                            as={as}
                            className={`app-header__menu ${is_active ? 'app-header__menu--active' : ''}`}
